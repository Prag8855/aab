...
def test_download_buttons(page, assert_snapshot):
    ...
    download_1 = page.get_by_role(
        "button", name="Download your Anmeldung form (part 1)"
    )
    expect(download_1).to_contain_text(people[0]["first_name"])
    expect(download_1).to_contain_text(people[1]["first_name"])

    download_2 = page.get_by_role(
        "button", name="Download your Anmeldung form (part 2)"
    )
    expect(download_2).to_contain_text(people[2]["first_name"])
    expect(download_2).to_contain_text(people[3]["first_name"])

    download_3 = page.get_by_role(
        "button", name="Download your Anmeldung form (part 3)"
    )
