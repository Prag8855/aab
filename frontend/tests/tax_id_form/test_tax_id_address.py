...
def test_data_remembered_no_anmeldung(page):
    fill_tax_id_form_until(
        page, "address", purpose="I can't register my address, but I need a tax ID"
    )
...
def test_data_remembered_living_abroad(page):
    fill_tax_id_form_until(
        page, "address", purpose="I don't live in Germany, but I need a tax ID"
    )
    ...
    expect(page.get_by_label("Street address")).to_have_value(address["street"])
    expect(page.get_by_label("City and post code")).to_have_value(
...
def test_data_validity_check(page):
    ...
    expect(page.locator(".tax-id-form")).not_to_have_class(
